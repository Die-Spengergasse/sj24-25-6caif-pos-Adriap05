...
namespace SPG_Fachtheorie.Aufgabe3.Test
{
    ...
    {
        public async Task GetAllPaymentsTests(int CashDesk, HttpStatusCode expectedStatusCode)
        {
            // ARRANGE
            var factory = new TestWebApplicationFactory();
            factory.InitializeDatabase(db =>
            {
                var cashDesk = new CashDesk(1); // richtig ???
            ...
            // ACT
            var (statusCode, content) = await factory.GetHttpContent<List<PaymentDto>>($"/api/payments?cashDesk=1");

            // ASSERT
            Assert.True(statusCode == System.Net.HttpStatusCode.OK);
            Assert.NotNull(content);
            Assert.True(content.First().R);
